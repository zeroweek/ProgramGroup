 ...
 *     function. 
 */  
DECLARE size_t LPAPI lpUTF8ToWchar(const char *in, size_t insize, wchar_t *out, size_t outsize,
    int flags)  
{  
    u_char *p, *lim;  
    wchar_t *wlim, high;  
 ...
 *  as regular symbols. 
 */  
DECLARE size_t LPAPI lpWcharToUTF8(const wchar_t *in, size_t insize, char *out, size_t outsize,
    int flags)  
{  
    wchar_t *w, *wlim, ch;  
    u_char *p, *lim, *oc;  
...
DECLARE BOOL LPAPI lpUTF8ToGB2312(const char *pText, LPUINT32 dwSize, string &pOut)
{
	...
	//注：
	//cbMultiByte填-1时，MultiByteToWideChar会把lpMultiByteStr的终止符算上
	//cchWideChar填-1时，WideCharToMultiByte会把lpWideCharStr的终止符算上

	if (0 == dwSize)
	{
		nLen = MultiByteToWideChar(CP_UTF8, 0, pText, -1, NULL, 0);
	}
	else
	{
		nLen = MultiByteToWideChar(CP_UTF8, 0, pText, dwSize, NULL, 0);
	}
	LOG_PROCESS_ERROR(nLen);

	pwStr = new wchar_t[nLen + 1];
	LOG_PROCESS_ERROR(pwStr);
	pwStr[nLen] = '\0';
	if (0 == dwSize)
	{
		nResult = MultiByteToWideChar(CP_UTF8, 0, pText, -1, pwStr, nLen);
	}
	else
	{
		nResult = MultiByteToWideChar(CP_UTF8, 0, pText, dwSize, pwStr, nLen);
	}
	...
	LOG_PROCESS_ERROR(pStr);
	pStr[nLen] = '\0';
	nResult = WideCharToMultiByte(CP_ACP, 0, pwStr, -1, pStr, nLen, NULL, NULL);
	LOG_PROCESS_ERROR(nResult == nLen);

	pOut = pStr;

	SAFE_DELETE_SZ(pwStr);
	SAFE_DELETE_SZ(pStr);
...
DECLARE BOOL LPAPI lpGB2312ToUTF8(const char *pText, LPUINT32 dwSize, string& pOut)
{
	...
	if (0 == dwSize)
	{
		nLen = MultiByteToWideChar(CP_ACP, 0, pText, -1, NULL, 0);
	}
	else
	{
		nLen = MultiByteToWideChar(CP_ACP, 0, pText, dwSize, NULL, 0);
	}
	LOG_PROCESS_ERROR(nLen);
	...
	if (0 == dwSize)
	{
		nResult = MultiByteToWideChar(CP_ACP, 0, pText, -1, pwStr, nLen);
	}
	else
	{
		nResult = MultiByteToWideChar(CP_ACP, 0, pText, dwSize, pwStr, nLen);
	}
	LOG_PROCESS_ERROR(nResult == nLen);

	nLen = WideCharToMultiByte(CP_UTF8, 0, pwStr, -1, NULL, 0, NULL, NULL);
	LOG_PROCESS_ERROR(nLen);

	pStr = new char[nLen + 1];
	LOG_PROCESS_ERROR(pStr);
	pStr[nLen] = '\0';
	nResult = WideCharToMultiByte(CP_UTF8, 0, pwStr, -1, pStr, nLen, NULL, NULL);
	LOG_PROCESS_ERROR(nResult == nLen);
